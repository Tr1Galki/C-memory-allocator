...
static bool test_allocate_block() {
    ...
    if (!heap) {
        fprintf(stderr, "test_allocate_block error!\n");
        return false;
    }
    ...
    fprintf(stderr, "test_allocate_block completed");
    return true;
}
...
static bool test_allocate_two_blocks() {
    void *heap = heap_init(HEAP_SIZE);
    if (!heap) {
        fprintf(stderr, "test_allocate_two_blocks error!\n");
        return false;
    }
    ...
    if (!first_block) {
        if (!second_block) {
            fprintf(stderr, "test_allocate_two_blocks: both blocks corrupted!\n");
        } else {
            fprintf(stderr, "test_allocate_two_blocks: first block corrupted!\n");
        }
        return false;
    } else if (!second_block) {
        fprintf(stderr, "test_allocate_two_blocks: second block corrupted!\n");
        return false;
    }

    memory_free(heap, first_block);
    fprintf(stderr, "test_allocate_two_blocks completed");
    return true;
}
...
static bool test_allocate_three_different_blocks() {
    void *heap = heap_init(HEAP_SIZE);
    if (!heap) {
        fprintf(stderr, "test_allocate_three_different_blocks error!\n");
        return false;
    }
    ...
    if (!first_block) {
        if (!second_block) {
            if (!third_block) {
                fprintf(stderr, "test_allocate_three_different_blocks: all blocks corrupted!\n");
            } else {
                fprintf(stderr, "test_allocate_three_different_blocks: first and second blocks corrupted!\n");
            }
        } else {
            if (!third_block) {
                fprintf(stderr, "test_allocate_three_different_blocks: first and third blocks corrupted!\n");
            } else {
                fprintf(stderr, "test_allocate_three_different_blocks: first block corrupted!\n");
            }
        }
        return false;
    } else if (!second_block) {
        if (!third_block) {
            fprintf(stderr, "test_allocate_three_different_blocks: second and third blocks corrupted!\n");
        } else {
            fprintf(stderr, "test_allocate_three_different_blocks: second block corrupted!\n");
        }
        return false;
    } else if (!third_block) {
        fprintf(stderr, "test_allocate_three_different_blocks: third block corrupted!\n");
        return false;
    }
